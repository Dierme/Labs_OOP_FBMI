...
int count_repetitions(Node<user_class>* list_1_head, Node<user_class>* list_2_head)
{
	int repetitions = 0;
	bool t;
...
template <class user_class>
bool check(Node<user_class>*head, user_class k)
{
...
template <class user_class>
List<user_class>::List(int n) :Head(NULL), Tail(NULL), size(n)
{
	srand(time(NULL));
	user_class *array;
	array = new int[size];
